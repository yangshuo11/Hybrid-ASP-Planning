...
namespace KCL_rosplan {
	...
	void RPActionInterface::runActionInterface() {
		...
		// subscribe to aruco detection result
        aruco_detect_client_ = nh.serviceClient<aruco_msgs::CheckActionEffect>("/aruco_single/action_effect_check");
	...
	void RPActionInterface::arucoDetectionResultCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
        aruco_detected = true;
	}
	...
	void RPActionInterface::dispatchCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg) {
        ...
        action_finished = concreteCallback(msg);
        ...
        if (action_finished)
        {
            ...
            aruco_msgs::CheckActionEffect srv;
            srv.request.aruco_id = 582;
            if (aruco_detect_client_.call(srv))
            {
                std::cout << "aruco_detect_srv_response: " << (bool)srv.response.detection_result << std::endl;
                if (srv.response.detection_result == true)
                {
                    ROS_INFO("Aruco detected! ");
                    ROS_INFO("KCL: (%s) action succeeded", params.name.c_str());
                    action_success = true;
                }
                else
                {
                    action_success = false;
                    ROS_INFO("KCL: (%s) action failed", params.name.c_str());
                    ROS_INFO("Notify the ASP solver for sensing planning");
                    // notify the ASP solver for planning a set of sensing behaviors
                    ...
                    asp_request_goal.request = "visit(red,o0)";
            ...
            {
                ROS_INFO("aruco_detect_clinet failed to call service");
            }
        }
        else
        {
            ROS_INFO("KCL: (%s) action not yet finished", params.name.c_str());
        }
